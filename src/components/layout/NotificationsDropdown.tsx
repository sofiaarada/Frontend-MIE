...
import { Bell, AlertTriangle, Info, CheckCircle2, Send } from 'lucide-react';
        ...
        {abierto && (
          ...
          >
            <div className="flex items-center justify-between border-b border-surface-100 px-4 py-3 dark:border-surface-800">
              ...
              {noLeidas > 0 && <button onClick={async () => { await notificationService.marcarTodasLeidas(); queryClient.invalidateQueries({ queryKey: ['notificaciones'] }); }} className="text-xs text-primary-600 hover:underline">Marcar todas leídas</button>}
            </div>
            <div className="max-h-80 overflow-y-auto">
              {notificaciones.map((n) => {
                const Icono = iconoPorTipo[n.tipo as keyof typeof iconoPorTipo];
                return (
                  <button type="button" onClick={() => !n.leido && marcarLeida(n.id)} key={n.id} className={cn('flex w-full gap-3 px-4 py-3 text-left', !n.leido && 'bg-primary-50/40 dark:bg-primary-500/5')}>
                    <div className={cn('flex h-8 w-8 shrink-0 items-center justify-center rounded-full', colorPorTipo[n.tipo as keyof typeof colorPorTipo])}>
