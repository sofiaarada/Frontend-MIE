...
export function UsuarioFormModal({ abierto, onCerrar, onGuardar, usuario, roles }: Props) {
  ...
  const { data: instituciones = [] } = useQuery({ queryKey: ['instituciones-combobox'], queryFn: async () => { const resultado = await resourcesApi.listar<{ id_institucion: string; nombre_institucion: string }>('instituciones', { pageSize: 1000 }); return resultado.data.map((i) => ({ id: String(i.id_institucion), nombre: i.nombre_institucion })); } });
