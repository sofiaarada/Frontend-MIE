...
import type { RolSistema, UsuarioAdmin, UsuarioInput } from '@/services/usuariosService';
...
interface Props { abierto: boolean; onCerrar: () => void; onGuardar: (valores: UsuarioInput) => Promise<void>; usuario?: UsuarioAdmin | null; roles: RolSistema[]; }
...
export function UsuarioFormModal({ abierto, onCerrar, onGuardar, usuario, roles }: Props) {
  ...
  useEffect(() => { if (abierto) reset(usuario ? { ...usuario, password: '' } : { ...empty, id_rol: roles[0]?.id_rol ?? 0 }); }, [abierto, usuario, roles, reset]);
  const submit = async (values: UsuarioFormValues) => { if (!usuario && (!values.password || values.password.length < 8)) { setError('password', { message: 'La contraseña inicial debe tener al menos 8 caracteres.' }); return; } setGuardando(true); try { await onGuardar(values as UsuarioInput); onCerrar(); } finally { setGuardando(false); } };
