...
interface UsuarioDB extends Omit<UsuarioInput, 'id_institucion'> { id_usuario: string; id_institucion?: string | null; nombre_rol: string; avatar_url?: string | null; fecha_creacion: string; }

const mapUsuario = (db: UsuarioDB): UsuarioAdmin => ({
  id: db.id_usuario, nombre: `${db.nombres} ${db.apellidos}`, correo: db.email, rol: db.nombre_rol, sede: db.id_institucion ?? '', activo: db.estado === 'Activo', avatarUrl: db.avatar_url ?? undefined, creadoEn: db.fecha_creacion?.split('T')[0] ?? '',
