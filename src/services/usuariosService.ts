...
export interface UsuarioInput { tipo_documento: string; documento_id: string; nombres: string; apellidos: string; email: string; telefono?: string; id_rol: number; id_institucion?: string | number | null; estado: 'Activo' | 'Inactivo'; password?: string; }
...
interface UsuarioDB extends Omit<UsuarioInput, 'id_institucion'> { id_usuario: string; id_institucion?: string | null; nombre_institucion?: string | null; nombre_rol: string; avatar_url?: string | null; fecha_creacion: string; }

const mapUsuario = (db: UsuarioDB): UsuarioAdmin => ({
  id: String(db.id_usuario), nombre: `${db.nombres} ${db.apellidos}`, correo: db.email, rol: db.nombre_rol, sede: db.nombre_institucion ?? '', activo: db.estado === 'Activo', avatarUrl: db.avatar_url ?? undefined, creadoEn: db.fecha_creacion?.split('T')[0] ?? '',
